...
# ---- F1-optimal threshold keresése a VALIDÁCIÓN ----
...
# =========================
# 11) EDGE-TÍPUS ABLÁCIÓ + DIAGRAM
# =========================

from copy import deepcopy

def make_variant_graphs(graphs, allowed_edge_types):
    """
    Csak azokat az éleket tartjuk meg, amelyeknek a típusa benne van az allowed_edge_types halmazban.
    A node-listát változatlanul átvesszük.
    """
    out = []
    for g in graphs:
        # fontos: node-okból elég a referencia, nem módosítjuk
        new_edges = [(s, d, t) for (s, d, t) in g.edges if t in allowed_edge_types]
        out.append(ASTGraph(g.nodes, new_edges, g.label, g.raw))
    return out


def train_variant(name, graphs_train_src, graphs_val_src, graphs_test_src, allowed_edge_types):
    """
    Egy teljes futás egy adott él-konfigurációval:
      - AST csak parent
      - AST + next_sibling
      - AST + next_token
      - AST + next_sibling + next_token
    Visszaadja a legjobb VALID F1-et és a TEST metrikákat.
    """
    print(f"\n\n========== KONFIG: {name} ==========")
    print("Engedélyezett él-típusok:", allowed_edge_types)

    # 1) Gráfok él-szűrése
    g_tr = make_variant_graphs(graphs_train_src, allowed_edge_types)
    g_va = make_variant_graphs(graphs_val_src,   allowed_edge_types)
    g_te = make_variant_graphs(graphs_test_src,  allowed_edge_types)

    # 2) PyG konverzió (a meglévő to_pyg() függvényt használjuk)
    pyg_tr = to_pyg(g_tr)
    pyg_va = to_pyg(g_va)
    pyg_te = to_pyg(g_te)

    # 3) Loader-ek ehhez a variánshoz
    train_loader_v = DataLoader(pyg_tr, batch_size=BATCH_TRAIN, shuffle=True)
    val_loader_v   = DataLoader(pyg_va, batch_size=BATCH_EVAL)
    test_loader_v  = DataLoader(pyg_te, batch_size=BATCH_EVAL)

    # 4) Osztálysúly a BCE-hez (újraszámoljuk erre a variánsra)
    y_train_v = np.array([int(g.y.item()) for g in pyg_tr])
    pos_v = (y_train_v == 1).sum()
    neg_v = (y_train_v == 0).sum()
    pos_weight_v = torch.tensor([max(1.0, neg_v / max(1, pos_v))],
                                dtype=torch.float, device=device)
    print('pos_weight (BCE) ebben a konfigban:', float(pos_weight_v.item()))

    # 5) Modell + optimizer + loss inicializálása erre a variánsra
    global model, opt, crit, scaler   # a run()/evaluate() ezeket használja

    num_edge_types = len(EDGE_TYPES)  # továbbra is 3 (parent, next_sibling, next_token)
    model = GGNNClassifierFeatsNoEmb(
    ...
    crit = torch.nn.BCEWithLogitsLoss(pos_weight=pos_weight_v)
    scaler = torch.cuda.amp.GradScaler(enabled=(device.type == "cuda"))

    epochs = EPOCHS_GGNN
    best_val, best_state = 0.0, None

    # 6) Tréning + egyszerű validáció (thr=0.5)
    for epoch in range(1, epochs + 1):
        tr_loss, tr_acc = run(train_loader_v, train=True)
        va_acc, va_prec, va_rec, va_f1, _ = evaluate(val_loader_v, thr=0.5)
        if va_f1 > best_val:
            best_val, best_state = va_f1, deepcopy(model.state_dict())
        print(f"[{name}] epoch {epoch:02d} | train acc {tr_acc:.3f} | "
    ...
    # 7) Küszöbhangolás ezen a konfigon
    best_thr, best_val_f1 = find_best_threshold(val_loader_v)
    print(f"[{name}] Best VAL F1 @ thr={best_thr:.3f}: {best_val_f1:.3f}")

    # 8) Teszt értékelés
    te_acc, te_prec, te_rec, te_f1, te_cm = evaluate(test_loader_v, thr=best_thr)
    print(f"[{name}] TEST | acc: {te_acc:.3f} | prec: {te_prec:.3f} | "
          f"rec: {te_rec:.3f} | f1: {te_f1:.3f}")
...
# --- A különböző konfigurációk lefuttatása ---

edge_configs = {
    "AST (csak parent)":                 {"parent"},
    "AST + next_sibling":               {"parent", "next_sibling"},
    "AST + next_token":                 {"parent", "next_token"},
    "AST + next_sibling + next_token":  {"parent", "next_sibling", "next_token"},
}

results = {}

for name, allowed in edge_configs.items():
    res = train_variant(name, graphs_train, graphs_val, graphs_test, allowed)
    results[name] = res

print("\nEredmények konfigurációnként:")
for name, res in results.items():
    print(f"- {name}: TEST F1 = {res['test_f1']:.3f}, "
          f"VAL F1 = {res['val_f1']:.3f}, thr = {res['best_thr']:.3f}")


# --- Diagram: melyik él-konfig milyen F1-et hozott? ---

labels = list(results.keys())
test_f1s = [results[name]["test_f1"] for name in labels]
val_f1s  = [results[name]["val_f1"]  for name in labels]

plt.figure(figsize=(8, 5))
x = np.arange(len(labels))
width = 0.35

plt.bar(x - width/2, val_f1s,  width, label="VAL F1")
plt.bar(x + width/2, test_f1s, width, label="TEST F1")

plt.xticks(x, labels, rotation=20, ha="right")
plt.ylabel("F1 score")
plt.title("Sima AST vs. next_sibling / next_token él-típusok hozzájárulása")
plt.legend()
plt.tight_layout()
plt.savefig("edge_ablation_f1.png", dpi=150)
plt.close()

print("\nAblációs diagram mentve: edge_ablation_f1.png")
