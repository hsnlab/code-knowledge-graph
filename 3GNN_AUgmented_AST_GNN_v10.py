...
# =========================
# 5.5) (ÚJ) SHARDING: DF → több Parquet fájl; epochonként fájlról dolgozunk
# =========================
from pathlib import Path
import gc

def write_shards(df: pd.DataFrame, out_dir: str, prefix: str, rows_per_shard: int = 100_000):
    out = Path(out_dir); out.mkdir(parents=True, exist_ok=True)
    meta = []
    for i, start in enumerate(range(0, len(df), rows_per_shard)):
        part = df.iloc[start:start+rows_per_shard]
        path = out / f"{prefix}_part{i:03d}.parquet"
        part.to_parquet(path, index=False)
        cnt = part['label'].value_counts()
        meta.append({
            "path": str(path),
            "rows": int(len(part)),
            "pos": int(cnt.get(1, 0)),
...
train_meta = write_shards(df_train, "shards/train", "train", rows_per_shard=100_000)
val_meta   = write_shards(df_val,   "shards/val",   "val",   rows_per_shard=50_000)
test_meta  = write_shards(df_test,  "shards/test",  "test",  rows_per_shard=50_000)

# pos_weight a TELJES train alapján
train_pos = sum(m["pos"] for m in train_meta)
train_neg = sum(m["neg"] for m in train_meta)
pos_weight = torch.tensor([max(1.0, train_neg/max(1,train_pos))], dtype=torch.float, device=device)
print("pos_weight (BCE):", float(pos_weight))

# a nagy DF-eket elengedjük RAM-ból (shardokból fogunk olvasni)
del df_tmp, df_train, df_val, df_test
gc.collect()

# =========================
# 6) AUGMENTED AST ÉPÍTÉS (normalizált kódból!)  — marad
# =========================
@dataclass
class ASTGraph:
...
# =========================
# 7) PyG KONVERZIÓ + VOCAB  — kiegészítve shard-alapú vocab-gyűjtéssel és lazy betöltéssel
# =========================
class Vocab:
    def __init__(self):
        self.map = {}
    ...
    def freeze(self):
        if self.unk_id is None:
            self.map["<UNK>"] = len(self.map)
            self.unk_id = self.map["<UNK>"]
        self.frozen = True
...
def to_pyg_single(g):
    # az eredeti to_pyg logikája 1 gráfra
...
# --- vocab gyűjtése shardokról (RAM-barát) ---
def collect_type_vocab_from_shards(metas):
    for m in metas:
        df = pd.read_parquet(m["path"])
        for _, row in tqdm(df.iterrows(), total=len(df), desc=f"build vocab: {Path(m['path']).name}"):
...
collect_type_vocab_from_shards(train_meta)
collect_type_vocab_from_shards(val_meta)
collect_type_vocab_from_shards(test_meta)
type_vocab.freeze()

vocab_size = type_vocab.size()
print('PyG (lazy) | vocab_size =', vocab_size)

# --- Lazy dataset: shardból beolvasott DF sor -> PyG Data on-the-fly ---
class LazyGraphDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        g = ASTGraph(n,e,int(row['label']),code)
        return to_pyg_single(g)

pin = (device.type == "cuda")

def make_loader_from_parquet(parquet_path: str, batch_size: int, shuffle: bool):
    df_part = pd.read_parquet(parquet_path)
    ds = LazyGraphDataset(df_part)
    loader = DataLoader(ds, batch_size=batch_size, shuffle=shuffle,
                        pin_memory=pin, num_workers=4, persistent_workers=True)
    return loader
...
def find_best_threshold_sharded(val_meta):
    model.eval(); y_true, probs = [], []
    with torch.no_grad():
        with torch.cuda.amp.autocast(enabled=use_amp):
            for m in val_meta:
                loader = make_loader_from_parquet(m["path"], BATCH_EVAL, shuffle=False)
                for batch in loader:
                    batch = batch.to(device)
                    logits = model(batch)
...
# ---- Értékelés megadott küszöbbel (shardok felett) ----
def evaluate_sharded(meta, thr=0.5):
...
# =========================
# 10) TANÍTÁS + VALIDÁCIÓ + MENTÉS (BEST F1 ALAPJÁN) — shardonként
# =========================
best_val, best_state = 0.0, None
for epoch in range(1, epochs+1):
    # TRAIN: minden train shardon végigmegyünk
    random.shuffle(train_meta)
    tr_loss_sum = 0.0; tr_acc_sum = 0.0; tr_rows = 0
    for m in train_meta:
        loader = make_loader_from_parquet(m["path"], BATCH_TRAIN, shuffle=True)
        tr_loss, tr_acc = run(loader, train=True)
        tr_loss_sum += tr_loss * m["rows"]; tr_acc_sum += tr_acc * m["rows"]; tr_rows += m["rows"]
        del loader
    tr_loss = tr_loss_sum / max(1, tr_rows)
    tr_acc  = tr_acc_sum  / max(1, tr_rows)

    # VALIDATION: shardonként (nem keverünk)
    va_acc, _, _, va_f1, _ = evaluate_sharded(val_meta, thr=0.5)
    if va_f1 > best_val:
...
best_thr, best_val_f1 = find_best_threshold_sharded(val_meta)
print(f"Best VAL F1 @ thr={best_thr:.3f}: {best_val_f1:.3f}")

# --- végső TEST értékelés a megtalált küszöbbel ---
te_acc, te_prec, te_rec, te_f1, te_cm = evaluate_sharded(test_meta, thr=best_thr)
...
bundle = {
    ...
    "epoch": epoch,                      # hányadik epoch után készült
    "best_val_f1": float(best_val),      # legjobb VAL-F1, amihez a súly tartozik
    "best_thr": float(best_thr) if 'best_thr' in locals() else 0.5,  # F1-optimális küszöb
    "config": {                          # a modell újjáépítéséhez szükséges dimenziók
    ...
    "edge_types": EDGE_TYPES,            # hogy következetes legyen a betöltéskor
    "type_vocab": type_vocab.map,        # AST-típus -> id térkép (ha kell inferenciához)
    "saved_at": datetime.now().isoformat(timespec="seconds"),
}

bundle_name = f'checkpoint_bundle_{datetime.now():%Y%m%d_%H%M%S}.pt'
torch.save(bundle, bundle_name)
print('Mentve (bundle):', bundle_name)
