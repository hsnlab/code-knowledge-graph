...
class GGNNClassifierFeatsNoEmb(nn.Module):
    ...
    # 🔥 ÚJ: logit + node-szintű attention súlyok
    def forward_with_importance(self, data: Data):
        """
        Ugyanaz, mint a forward, de visszaadja az attention súlyokat is
        minden node-ra (egy gráf esetén: [N], összege ~1).
        """
        h = self.build_features(data)
        et = getattr(data, "edge_type", None)
        for blk in self.blocks:
        ...
        # Attention gate: ugyanaz a gate_nn, mint amit a GlobalAttention használ
        gate = self.pool.gate_nn(h).view(-1, 1)       # [N,1]
        alpha = softmax(gate, batch, dim=0)           # [N,1] node fontosság

        # Gráf embeddinget továbbra is a meglévő pool-lal számoljuk,
        # így a logit bittosan ugyanaz marad, mint korábban.
        hg = self.pool(h, batch)                      # [B, C]
        logits = self.head(hg).view(-1)               # [B]

        return logits, alpha.view(-1)                 # (logitek, node_importance [N])
...
def build_single_data(code: str, vocab_map: Dict[str, int], tok_dim: int):
    norm = normalize_code(code)
    nodes, edges = build_augmented_ast(norm)
    data = graph_to_pyg(nodes, edges, vocab_map, tok_dim)
    # egyetlen gráf: batch = 0..0
...
def predict_with_explanations(code: str, model_path: str, meta_path: str, top_k: int = 10):
    meta = load_meta(meta_path)
    vocab_map: Dict[str, int] = {str(k): int(v) for k, v in meta["vocab_map"].items()}
    tok_dim: int = int(meta["tok_dim"])

    # gráf + node-meta
    data, nodes = build_single_data(code, vocab_map, tok_dim)

    # modell hyperparaméterek a meta-ból
    ...
    with torch.no_grad():
        logits, node_alpha = model.forward_with_importance(data)

    prob = sigmoid_probability(logits)
    node_alpha = node_alpha.detach().cpu().numpy()   # shape [N]

    N = len(node_alpha)
    top_k = min(top_k, N)
    top_idx = np.argsort(node_alpha)[::-1][:top_k]

    important_nodes = []
    for idx in top_idx:
        n = nodes[idx]
        important_nodes.append({
            "index": int(idx),                        # index a gráfban
            "id": int(n.get("id", idx)),             # az AST-ben használt id (ugyanaz)
            "importance": float(node_alpha[idx]),    # attention súly
            "type": n.get("type", ""),
            "text": n.get("text", ""),
            "depth": int(n.get("depth", 0)),
            "is_leaf": int(n.get("is_leaf", 0)),
        })

    return prob, important_nodes
...
def main():
    ...
    ap.add_argument("--topk", type=int, default=10, help="How many top important nodes to return")
    args = ap.parse_args()
    ...
    prob, important_nodes = predict_with_explanations(
        code,
    ...
    # JSON output – könnyen parse-olható
