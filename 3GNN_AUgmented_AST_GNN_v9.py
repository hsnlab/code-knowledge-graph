...
# --- (Opcionális) Rebalancing függvény; most NINCS használva ---
...
print('Minták száma összesen:', len(raw_df))
...
def normalize_code(code: str) -> str:
    ...
    try:
        tree = parser.parse(text); root = tree.root_node
    except Exception:
        import re
    ...
    def push_scope(): scope_stack.append({"func":None, "params":{}, "vars":{}})
    def pop_scope(): scope_stack.pop()
    def get_id(node): return text[node.start_byte:node.end_byte].decode("utf8","ignore")
    def assign_param(name, sc):
        if name in sc["params"]: return sc["params"][name]
        idx = len(sc["params"]) + 1; sc["params"][name] = f"PARAM_{idx}"; return sc["params"][name]
    def assign_var(name, sc):
    ...
    def first_pass(node):
        ...
        if t == "function_definition":
            ...
            for ch in node.children:
                if ch.type == "function_declarator": func_decl = ch; break
            if func_decl is not None:
                for ch in func_decl.children:
                    if ch.type == "identifier":
                        counters["func"] += 1; scope_stack[-1]["func"] = f"FUNC_{counters['func']}"; break
                for ch in func_decl.children:
                    if ch.type == "parameter_list":
                        for p in ch.children:
                            if p.type == "parameter_declaration":
                                for gch in p.children:
                                    if gch.type == "identifier": assign_param(get_id(gch), scope_stack[-1])
            for ch in node.children: first_pass(ch)
            pop_scope(); return
        if scope_stack:
            if t in {"init_declarator","declarator"}:
                for ch in node.children:
                    if ch.type == "identifier": assign_var(get_id(ch), scope_stack[-1])
        for ch in node.children: first_pass(ch)
    ...
    def second_pass(node):
        ...
        if t == "function_definition":
            push_scope(); out=[]
            for ch in node.children: out.append(second_pass(ch))
            pop_scope(); return "".join(out)
        if t in LITS: return LITS[t]
        if t == "identifier":
            if scope_stack and scope_stack[-1]["func"] is not None:
                parent = node.parent
                if parent and parent.type == "function_declarator": return scope_stack[-1]["func"]
            return lookup_identifier(get_id(node))
        if len(node.children) == 0:
            return text[node.start_byte:node.end_byte].decode("utf8","ignore")
        parts = [second_pass(ch) for ch in node.children]
        return "".join(parts)
...
# =========================
# 7) VOCAB + LAZA (ON-THE-FLY) DATASET, HOGY NE FOGYJON EL A RAM
# =========================
class Vocab:
    def __init__(self):
...
def collect_type_vocab(*dfs):
    for df in dfs:
        for _, row in tqdm(df.iterrows(), total=len(df), desc="build vocab"):
...
def to_pyg_single(g: ASTGraph) -> Data:
    ...
    if len(g.edges)==0:
        edge_index = torch.empty((2,0), dtype=torch.long); edge_type = torch.empty((0,), dtype=torch.long)
    else:
...
# --- vocab felépítése RAM-barát módon ---
collect_type_vocab(df_train, df_val, df_test)
vocab_size = type_vocab.size()
print('vocab_size =', vocab_size)

# --- DataLoaderek (on-the-fly építés) ---
pin = (device.type == "cuda")
train_ds, val_ds, test_ds = LazyGraphDataset(df_train), LazyGraphDataset(df_val), LazyGraphDataset(df_test)
...
# Osztálysúly BCE-hez: pos_weight = neg/pos  (közvetlenül a DF-ből)
...
# =========================
# 8) GGNN + LeakyReLU + Attention Pooling
# =========================
...
# --- TELJES CHECKPOINT (bundle) ---
bundle = {
    "state_dict": model.state_dict(),
    "optimizer": opt.state_dict(),
    "epoch": epoch,
    "best_val_f1": float(best_val),
    "best_thr": float(best_thr) if 'best_thr' in locals() else 0.5,
    "config": {
