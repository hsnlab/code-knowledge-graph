...
# =========================
# 9) EGY EXPERIMENT FUTTATÁSA 1 SEED-DEL
# =========================
def run_experiment(seed: int):
    """
    Teljes pipeline 1 seedre:
      - dataset betöltés + balanszírozás
      - stratifikált split
      - AST gráf építés + PyG konverzió
      - GGNN tréning
      - küszöb hangolás
      - test kiértékelés
      - modell + meta mentése seed-spezifikus néven
    """
    print(f"\n================== SEED = {seed} ==================")
    ...
    eda_df.describe(include='all').to_csv(os.path.join(EDA_DIR, f"eda_summary_describe_seed{seed}.csv"))
    raw_df['label'].value_counts().to_csv(os.path.join(EDA_DIR, f"label_counts_seed{seed}.csv"))
    raw_df['label'].value_counts(normalize=True).to_csv(os.path.join(EDA_DIR, f"label_proportions_seed{seed}.csv"))
    ...
    # --- 1) Label eloszlás ---
    _bar_counts(eda_df["label"], "Címke-eloszlás", "label", f"labels_bar_seed{seed}.png")

    # --- 2) Kódhossz (karakter) + sorok ---
    _hist(eda_df["len_chars"], "Kódhossz (karakter)", "chars", f"len_chars_hist_seed{seed}.png", bins=60, logx=True)
    _hist(eda_df["num_lines"], "Sorszám a forrásban", "lines", f"num_lines_hist_seed{seed}.png", bins=60, logx=True)

    # --- 3) Normalizált kódhossz ---
    _hist(eda_df["len_chars_norm"], "Normalizált kódhossz (karakter)", "chars_norm", f"len_chars_norm_hist_seed{seed}.png", bins=60, logx=True)

    # --- 4) Egyszerű tartalmi arányok és számlálók ---
    _hist(eda_df["digit_ratio"], "Számjegy arány", "digit_ratio", f"digit_ratio_hist_seed{seed}.png", bins=50)
    _hist(eda_df["upper_ratio"], "Nagybetű arány", "upper_ratio", f"upper_ratio_hist_seed{seed}.png", bins=50)
    _hist(eda_df["lower_ratio"], "Kisbetű arány", "lower_ratio", f"lower_ratio_hist_seed{seed}.png", bins=50)
    _hist(eda_df["sym_ratio"],   "Szimbolikus jelek aránya", "sym_ratio", f"sym_ratio_hist_seed{seed}.png", bins=50)
    _hist(eda_df["num_string_literals"], "String literálok száma", "num_string_literals", f"num_string_literals_hist_seed{seed}.png", bins=50, logx=True)
    _hist(eda_df["num_char_literals"],   "Char literálok száma",   "num_char_literals",   f"num_char_literals_hist_seed{seed}.png", bins=50, logx=True)
    ...
    _corr_heatmap(eda_df[num_cols], "Korrelációs mátrix (numerikus jellemzők)", f"corr_matrix_seed{seed}.png")

    print(f"[EDA] Kész! Fájlok az '{EDA_DIR}/' mappában (seed={seed}).")
    ...
    for epoch in range(1, epochs+1):
        ...
        print(f"[seed {seed}] epoch {epoch:02d} | train acc {tr_acc:.3f} | val acc {va_acc:.3f} | val F1 {va_f1:.3f}")
    ...
    print(f"[seed {seed}] Best VAL F1 @ thr={best_thr:.3f}: {best_val_f1:.3f}")

    # --- végső TEST értékelés a megtalált küszöbbel ---
    te_acc, te_prec, te_rec, te_f1, te_cm = evaluate(test_loader, thr=best_thr)
    print(f"[seed {seed}] TEST | acc: {te_acc:.4f} | prec: {te_prec:.4f} | rec: {te_rec:.4f} | f1: {te_f1:.4f}")
    print(f"[seed {seed}] Confusion matrix:\n", te_cm)

    out_name = f'cpp_augast_GGNN_bce_best_seed{seed}.pt'
    ...
    meta_name = f"cpp_augast_meta_seed{seed}.json"
    with open(meta_name, "w", encoding="utf-8") as f:
        json.dump(meta, f)
    print("Mentve:", meta_name)

    print(f"[seed {seed}] vocab size:", len(meta["vocab_map"]), "| tok_dim:", meta["tok_dim"], "| num_edge_types:", meta["num_edge_types"])

    # eredmények visszaadása összegzéshez
    return {
        "seed": seed,
...
# =========================
# 11) TÖBB SEED FUTTATÁSA
# =========================
if __name__ == "__main__":
    seeds = [1, 2, 3, 10, 21, 42]   # itt szabadon bővítheted / módosíthatod
    all_results = []
    for s in seeds:
        res = run_experiment(s)
        all_results.append(res)

    print("\n================== ÖSSZEFOGLALÓ (SEED SWEEP) ==================")
    for r in all_results:
        print(
            f"seed={r['seed']} | "
            f"test_acc={r['test_acc']:.4f} | "
            f"test_prec={r['test_prec']:.4f} | "
            f"test_rec={r['test_rec']:.4f} | "
            f"test_f1={r['test_f1']:.4f} | "
            f"best_val_f1={r['best_val_f1']:.4f} | "
            f"thr={r['best_thr']:.3f}"
        )

    # opcionálisan mentheted egy JSON-ba is:
    summary_path = "seed_sweep_results.json"
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(all_results, f, indent=2)
    print("Seed sweep eredmények mentve ide:", summary_path)
