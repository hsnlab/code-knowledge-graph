...
from torch_geometric.loader import DataLoader, DataListLoader   # <<< DataListLoader is
...
device = pick_device()
print("device (kezdetben):", device)

# --- Multi-GPU flag ---
USE_MULTI_GPU = (torch.cuda.is_available() and torch.cuda.device_count() > 1)
if USE_MULTI_GPU:
    device = torch.device("cuda:0")
    print(f"Multi-GPU mód aktív, fő device: {device}, összes GPU: {torch.cuda.device_count()}")
else:
    if torch.cuda.is_available():
        print(f"Egyetlen CUDA GPU használata: {device}")
    else:
        print("Nincs CUDA GPU, CPU/MPS/XPU mód.")

print("Végleges device:", device)
...
def to_pyg(gs):
    pyg = []
    for g in gs:
        # EDA számlálás: node típusok gyakorisága
        try:
            # TYPE_FREQ globálisan gyűjti az AST node típusokat
            TYPE_FREQ.update([n['type'] for n in g.nodes])
        except Exception:
            pass
...
def plot_label_dist(df: pd.DataFrame, name: str):
    ...
    fig = plt.figure()
    ax = fig.gca()
    ax.bar(["non-vul (0)", "vuln (1)"], cnt.tolist())
    title_text = (
        f"Label distribution – {name} "
        f"N={len(df)} | pos={int(cnt.get(1,0))} "
        f"({props.get(1,0)*100:.1f}%)"
    )
    ax.set_title(title_text)
    ax.set_ylabel("count")
    fig.tight_layout()
    fig.savefig(Path(EDA_DIR)/f"labels_{name}.png")
    plt.close(fig)

def plot_code_len_hist(df: pd.DataFrame, name: str):
    lens = df['code'].astype(str).map(len).clip(upper=20000)
    fig = plt.figure()
    ax = fig.gca()
    ax.hist(lens, bins=50)
    ax.set_title(f"Code length (chars) – {name}")
    ax.set_xlabel("length (clipped at 20k)")
    ax.set_ylabel("count")
    fig.tight_layout()
    fig.savefig(Path(EDA_DIR)/f"codelen_{name}.png")
    plt.close(fig)
...
plot_label_dist(df_val,   "val")
plot_label_dist(df_test,  "test")
plot_label_dist(raw_df,   "all")

plot_code_len_hist(df_train, "train")
plot_code_len_hist(df_val,   "val")
plot_code_len_hist(df_test,  "test")
plot_code_len_hist(raw_df,   "all")

# AST típusok: a to_pyg közben gyűlnek → most kirajzoljuk
plot_top_ast_types(25)

# =========================
# 11) BIZTONSÁGOS TORCH.LOAD (PyTorch 2.6+)
# =========================
...
# =========================
# 12) MODELL DEFINÍCIÓK
# =========================

import torch.nn.functional as F
from torch import nn
from torch_geometric.nn import GatedGraphConv, GlobalAttention, DataParallel   # <<< DataParallel
...
class GGNNClassifierFeatsNoEmb(nn.Module):
    ...
    def forward(self, data):
        ...
        h = self.drop(h)
        # batch attribútum biztosítása
        if not hasattr(data, 'batch') or data.batch is None:
            data.batch = torch.zeros(h.size(0), dtype=torch.long, device=h.device)
...
base_model = GGNNClassifierFeatsNoEmb(
    num_types=vocab_size, tok_dim=TOK_DIM, small_dim=2,
    steps=10, blocks=5, num_edge_types=num_edge_types, dropout=0.3
).to(device)

if USE_MULTI_GPU:
    model = DataParallel(base_model, device_ids=list(range(torch.cuda.device_count())))
    print("GGNN modell DataParallel-ben, GPU-k:", list(range(torch.cuda.device_count())))
else:
    model = base_model
    print("GGNN modell single-device módon.")
...
# =========================
# 14) SHARDOS TRAIN/EVAL FUNKCIÓK (Multi-GPU kompatibilisen)
# =========================
...
def run_loader(pyg_list, train: bool):
    ...
    if USE_MULTI_GPU:
        loader = DataListLoader(pyg_list, batch_size=BATCH_TRAIN if train else BATCH_EVAL, shuffle=train)
    else:
    ...
    for batch in loader:
        if USE_MULTI_GPU:
            data_input = batch           # list[Data] → DataParallel intézi a GPU-kra pakolást
        else:
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            ...
            if USE_MULTI_GPU:
                ys = [d.y for d in batch]
                target = torch.cat(ys, dim=0).float().view(-1).to(device)
        ...
        if USE_MULTI_GPU:
            correct += int((pred.detach().cpu() == target.detach().cpu().long()).sum())
        else:
...
def collect_probs_and_labels_from_shards(shard_paths):
    ...
    for p in shard_paths:
        ...
        if USE_MULTI_GPU:
            loader = DataListLoader(pyg_list, batch_size=BATCH_EVAL, shuffle=False)
        else:
        ...
        for batch in loader:
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                ...
                if USE_MULTI_GPU:
                    ys = [d.y for d in batch]
                    y_true += torch.cat(ys, dim=0).cpu().tolist()
                else:
...
# =========================
# 15) TANÍTÁS + VALIDÁCIÓ SHARDONKÉNT
# =========================
...
for epoch in range(1, EPOCHS_GGNN+1):
    ...
    if f1 > best_val_f1:
        best_val_f1 = f1
        best_state  = base_model.state_dict()   # <<< mindig a base_model állapotát mentjük
...
best_thr, best_val_f1_opt = find_best_thr_on_val()
print(f"Best VAL F1 @ thr={best_thr:.3f}: {best_val_f1_opt:.3f}")
...
# csak a base_model-t mentjük, DataParallel nélkül
torch.save(base_model.state_dict(), out_name)
